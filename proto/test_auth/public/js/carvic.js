...
Carvic.Utils = {
    ...
    //LoadNodeStatusList: function (receiver) {
    //    var query = {};
    ...
    LoadClusterList: function (receiver) {
        var query = {};
        Carvic.Utils.Post({ action: "get_clusters", data: query }, function (data) {
            for (var i = 0; i < data.length; i++) {
                var obj = data[i];
    ...
    Logout: function () {
        ...
        try {
            ...
            if (agt.indexOf("msie") != -1) {
                ...
                xmlhttp.open("GET", "logout", true, "logoutxxx", "logoutxxxx");
                xmlhttp.send("");
                xmlhttp.abort();
            }
        } catch (e) {
...
Carvic.Model.UsersModel = function () {
    ...
    self.SaveNewUser = function () {
        if (self.NewUserPwd1() !== self.NewUserPwd1()) {
...
Carvic.Model.NodesModel = function () {
    ...
    // load cluster list
    Carvic.Utils.LoadClusterList(self.NodeSearchClusterList);
    ...
    self.SearchInner = function (reset_page) {
        ...
        if (self.NodeSearchId() != "") { query.id = Number(self.NodeSearchId()); }
        ...
        if (self.NodeSearchCluster() != "") { query.cluster = self.NodeSearchCluster(); }
...
Carvic.Model.SingleNodeModel = function () {
    ...
    self.LoadDataFromObject = function (data) {
        ...
        self.NodeMapUrl("map.html?lat=" + encodeURI(data.loc_lat) + "&lon=" + encodeURI(data.loc_lon) + "&title=" + encodeURI(data.name));
        self.NodeSN(data.sn);
    ...
    self.EndEditNode = function () {
        ...
        var components = [];
        self.Components().forEach(function (item) { components.push(item.Id()); });
        var req = {
            action: "update_node",
            data: {
                id: self.NodeID(),
...
Carvic.Model.NewNodeModel = function () {
    ...
    self.LoadLastNode = function () {
        ...
        Carvic.Utils.Post(req, function (data) {
            // TODO preveri za katere je smiselno uporabljati zadnjo vrednost
...
Carvic.InitNodeList = function () {
    Carvic.Model.Nodes = new Carvic.Model.NodesModel();
